                ...
                .then(async (result) => {
                  if (req.body.role === "hospital") {
                    ...
                    return res.status(200).send({
                      status: 1,
                      message: "Hospital has been registered successfully.",
                      data: {
                        token: data.user_authentication,
                        user_id: data._id,
                    ...
                    const doc = new Document({
                      ...
                      "cnaCertificate.name": "cnaCertificate",
                      "nursingLicense.name": "nursingLicense",
                      "socialSecurity.name": "socialSecurity",
                      "cpr.name": "cpr",
                      "bls.name": "bls",
                      "acls.name": "acls",
                      "covidVaccination.name": "covidVaccination",
                      "hepatitesSeries.name": "hepatitesSeries",
                      user_id: result._id,
                    });
                    await doc.save();
  ...
  if (!req.body.user_id) {
    ...
    User.find({ _id: req.body.user_id })
      ...
      .then((result) => {
        ...
        User.findByIdAndUpdate(
          req.body.user_id,
          { verified: 0, verification_code: 123456 },
          (err, _result) => {
            if (err) {
              res.status(400).send({
                status: 0,
